...
export function DemoIntro() {
  ...
  return <section className="ki-story" aria-label="社内ナレッジAIの使い方">
    <div className="ki-story-top"><span>使い方を見てみる</span><span>約36秒 · サンプル規程での紹介</span></div>
    <div className="ki-viewport" ref={viewport} data-scene={index} data-time={Math.round(time)} data-paused={paused || reduced}>
      <div className="ki-stage" aria-hidden="true" inert style={{ transform: `translate(${width / 2 - camera[0] * scale}px, ${168 - camera[1] * scale}px) scale(${scale})`, opacity: index === 6 ? .23 : 1 }}>
        <IntroScreens phase={index} sent={index > 0 || current.elapsed >= 1600} stars={stars} />
      </div>
      {index === 6 ? <div className="ki-summary"><span>社内の知識を、次の行動へ。</span><h3>質問から、根拠の確認まで。</h3><div><span>質問する</span><b>→</b><span>規程を確認</span><b>→</b><span>手続きが分かる</span></div><p>不足情報は確認し、資料にないことは推測しません。</p></div> : null}
      <div className="ki-hud"><div className="ki-dots" aria-hidden="true">{scenes.map((scene, i) => <span key={scene.title} className={i === index ? "ki-current" : ""} />)}</div><p>{scenes[index].caption}</p></div>
    </div>
